...
type Config struct {
	...
	// AIStudioKey defines AI Studio API key configurations (Portkey-backed).
	AIStudioKey []AIStudioKey `yaml:"ai-studio-key" json:"ai-studio-key"`
...
// AIStudioKey represents the configuration for an AI Studio API key (Portkey-backed).
type AIStudioKey struct {
	// APIKey is the authentication key for accessing AI Studio/Gemini API.
	APIKey string `yaml:"api-key" json:"api-key"`
	...
	// BaseURL optionally overrides the AI Studio API endpoint (defaults to Portkey).
	BaseURL string `yaml:"base-url,omitempty" json:"base-url,omitempty"`
	...
	// Models defines upstream model names and aliases for request routing.
	Models []AIStudioModel `yaml:"models,omitempty" json:"models,omitempty"`
...
func (k AIStudioKey) GetAPIKey() string  { return k.APIKey }
func (k AIStudioKey) GetBaseURL() string { return k.BaseURL }

// AIStudioModel describes a mapping between an alias and the actual upstream model name.
...
func (m AIStudioModel) GetName() string  { return m.Name }
func (m AIStudioModel) GetAlias() string { return m.Alias }
...
func LoadConfigOptional(configFile string, optional bool) (*Config, error) {
	...
	// Sanitize AI Studio API key configuration.
	cfg.SanitizeAIStudioKeys()
...
// SanitizeAIStudioKeys deduplicates and normalizes AI Studio credentials.
func (cfg *Config) SanitizeAIStudioKeys() {
	if cfg == nil {
		return
	}

	seen := make(map[string]struct{}, len(cfg.AIStudioKey))
	out := cfg.AIStudioKey[:0]
	for i := range cfg.AIStudioKey {
		entry := cfg.AIStudioKey[i]
		entry.APIKey = strings.TrimSpace(entry.APIKey)
		if entry.APIKey == "" {
			continue
		}
