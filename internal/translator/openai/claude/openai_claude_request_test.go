...
func TestConvertClaudeRequestToOpenAI_ThinkingToReasoningContent(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !found {
				t.Fatal("Could not find a non-system message to check")
			}
...
func TestConvertClaudeRequestToOpenAI_ThinkingOnlyMessagePreserved(t *testing.T) {
	...
	// Should have: user + assistant (thinking-only) + user = 3 messages (system not auto-added if empty)
	if len(messages) != 3 {
...
func TestConvertClaudeRequestToOpenAI_ToolResultOrderAndContent(t *testing.T) {
	...
	// (System message not added if empty)
	if len(messages) != 3 {
...
func TestConvertClaudeRequestToOpenAI_AssistantTextToolUseTextOrder(t *testing.T) {
	...
	if len(messages) != 1 {
		t.Fatalf("Expected 1 messages, got %d. Messages: %s", len(messages), resultJSON.Get("messages").Raw)
	}
