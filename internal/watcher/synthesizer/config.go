...
func (s *ConfigSynthesizer) Synthesize(ctx *SynthesisContext) ([]*coreauth.Auth, error) {
	...
	// AI Studio (Portkey-backed)
	out = append(out, s.synthesizeAIStudioKeys(ctx)...)
...
// synthesizeAIStudioKeys creates Auth entries for AI Studio API keys.
func (s *ConfigSynthesizer) synthesizeAIStudioKeys(ctx *SynthesisContext) []*coreauth.Auth {
	cfg := ctx.Config
	now := ctx.Now
	idGen := ctx.IDGenerator

	out := make([]*coreauth.Auth, 0, len(cfg.AIStudioKey))
	for i := range cfg.AIStudioKey {
		...
		base := strings.TrimSpace(entry.BaseURL)
		proxyURL := strings.TrimSpace(entry.ProxyURL)
		id, token := idGen.Next("ai-studio:apikey", key, base)
		attrs := map[string]string{
			"source":  fmt.Sprintf("config:ai-studio[%s]", token),
			"api_key": key,
		}
		if entry.Priority != 0 {
			attrs["priority"] = strconv.Itoa(entry.Priority)
		...
		if hash := diff.ComputeGeminiModelsHash(convertAIStudioToGeminiModels(entry.Models)); hash != "" {
			attrs["models_hash"] = hash
		}
		addConfigHeadersToAttrs(entry.Headers, attrs)
		a := &coreauth.Auth{
...
func convertAIStudioToGeminiModels(models []config.AIStudioModel) []config.GeminiModel {
	out := make([]config.GeminiModel, len(models))
	for i, m := range models {
		out[i] = config.GeminiModel{Name: m.Name, Alias: m.Alias}
	}
	return out
}
