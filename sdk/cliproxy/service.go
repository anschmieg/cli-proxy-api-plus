...
func (s *Service) ensureExecutorsForAuth(a *coreauth.Auth) {
	...
	case "cloudcode":
		s.coreManager.RegisterExecutor(executor.NewCloudCodeExecutor(s.cfg))
	case "claude":
...
func (s *Service) registerModelsForAuth(a *coreauth.Auth) {
	...
	case "gemini-cli":
		...
		models = applyExcludedModels(models, excluded)
	case "cloudcode":
		// Combine Gemini CLI and Antigravity models for CloudCode
		models = registry.GetGeminiCLIModels()
		// Also fetch dynamic Antigravity models
		ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
		agModels := executor.FetchAntigravityModels(ctx, a, s.cfg)
		cancel()
		if len(agModels) > 0 {
			models = append(models, agModels...)
		}
		models = applyExcludedModels(models, excluded)
	case "aistudio":
	...
	case "ai-studio":
		models = registry.GetAIStudioModels()
		if entry := s.resolveConfigAIStudioKey(a); entry != nil {
			if len(entry.Models) > 0 {
				models = buildAIStudioConfigModels(entry)
			}
			excluded = entry.ExcludedModels
		}
		models = applyExcludedModels(models, excluded)
...
func (s *Service) resolveConfigAIStudioKey(auth *coreauth.Auth) *config.AIStudioKey {
	if auth == nil || s.cfg == nil {
		return nil
	}
	var attrKey, attrBase string
	...
	for i := range s.cfg.AIStudioKey {
		entry := &s.cfg.AIStudioKey[i]
		cfgKey := strings.TrimSpace(entry.APIKey)
		cfgBase := strings.TrimSpace(entry.BaseURL)
		if attrKey != "" && attrBase != "" {
			if strings.EqualFold(cfgKey, attrKey) && strings.EqualFold(cfgBase, attrBase) {
...
func buildAIStudioConfigModels(entry *config.AIStudioKey) []*ModelInfo {
	if entry == nil {
		return nil
	}
	return buildConfigModels(entry.Models, "google", "gemini")
